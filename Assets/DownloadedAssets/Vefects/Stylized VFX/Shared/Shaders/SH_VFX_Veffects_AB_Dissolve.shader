	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 Frag( VertexOutput packedInput ) : SV_Target
			{
				...
				float temp_output_43_0 = ( packedInput.ase_color.a * saturate( ( saturate( ( saturate( pow( tex2D( _Noise, texCoord68 ).r , _NoisePower ) ) * tex2D( _Texture, uv_Texture ).r ) ) + (0.0 + (( 1.0 - packedInput.ase_texcoord2.z ) - 0.0) * (1.0 - 0.0) / (1.0 - 0.0)) ) ) );
				
				surfaceDescription.Color = ( (packedInput.ase_color).rgb * temp_output_43_0 * packedInput.ase_texcoord2.w );
		...
		Pass
		{
					...
					)
			{
				...
				float temp_output_43_0 = ( packedInput.ase_color.a * saturate( ( saturate( ( saturate( pow( tex2D( _Noise, texCoord68 ).r , _NoisePower ) ) * tex2D( _Texture, uv_Texture ).r ) ) + (0.0 + (( 1.0 - packedInput.ase_texcoord1.z ) - 0.0) * (1.0 - 0.0) / (1.0 - 0.0)) ) ) );
		...
		Pass
		{
			...
			float4 Frag( VertexOutput packedInput  ) : SV_Target
			{
				...
				surfaceDescription.Color = ( (packedInput.ase_color).rgb * temp_output_43_0 * packedInput.ase_texcoord1.w );
				surfaceDescription.Emission = 0;
				surfaceDescription.Alpha = temp_output_43_0;
				surfaceDescription.AlphaClipThreshold =  _AlphaCutoff;
		...
		Pass
		{
					...
					)
			{
				...
				SurfaceDescription surfaceDescription = (SurfaceDescription)0;
				float2 texCoord68 = packedInput.ase_texcoord3.xy * _NoiseScale + ( _TimeParameters.x * _NoiseSpeed );
				float2 uv_Texture = packedInput.ase_texcoord3.xy * _Texture_ST.xy + _Texture_ST.zw;
				float temp_output_43_0 = ( packedInput.ase_color.a * saturate( ( saturate( ( saturate( pow( tex2D( _Noise, texCoord68 ).r , _NoisePower ) ) * tex2D( _Texture, uv_Texture ).r ) ) + (0.0 + (( 1.0 - packedInput.ase_texcoord4.z ) - 0.0) * (1.0 - 0.0) / (1.0 - 0.0)) ) ) );
...
/*ASEBEGIN
Version=18921
246;290;2546;1313;3440.304;1615.242;2.047332;True;True
...
WireConnection;76;2;43;0
ASEEND*/
//CHKSM=76DB7FDCB4FDA7E8B4D949FF261674AC544C0AC1